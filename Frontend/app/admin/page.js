...
function AdminDashboardContent() {
  ...
  const handleSendReport = async () => {
    ...
    try {
      const res = await API.post("/admin/analytics/send", { analytics: analyticsData });
      alert(res.data.message + (res.data.previewUrl ? `\n\nPreview URL (Ethereal): ${res.data.previewUrl}` : ""));
    } catch (err) {
            ...
            {activeTab === "analytics" && (
              <div>
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-2xl font-bold">Waste Analytics Report</h2>
                  <Button 
                    onClick={handleSendReport} 
                    disabled={sendingEmail || analyticsData.length === 0}
                    className="bg-primary hover:bg-primary/90 text-white"
                  >
                    {sendingEmail ? "Sending..." : "Send Report to Municipal"}
                  </Button>
                </div>
                ...
                {analyticsData.length === 0 ? (
                  ...
                  <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {analyticsData.map((areaData, idx) => (
                      <div key={idx} className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                        <h3 className="text-xl font-bold mb-2 text-primary">{areaData.area}</h3>
                        <p className="text-3xl font-semibold mb-4">{areaData.totalWaste} <span className="text-base font-normal text-gray-500">total reports</span></p>
                        
                        <div className="space-y-2">
                          <h4 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Breakdown</h4>
                          {areaData.breakdown.map((b, i) => {
                            const percent = ((b.count / areaData.totalWaste) * 100).toFixed(0);
                            return (
                              <div key={i} className="flex flex-col gap-1">
                                <div className="flex justify-between text-sm">
                                  <span>{b.type}</span>
                                  <span className="font-medium">{b.count} ({percent}%)</span>
                                </div>
                                <div className="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-1.5">
                                  <div 
                                    className={`h-1.5 rounded-full ${
                                      b.type === 'Wet' ? 'bg-green-500' :
                                      b.type === 'Dry' ? 'bg-blue-500' :
                                      b.type === 'E-Waste' ? 'bg-orange-500' : 'bg-gray-500'
                                    }`}
                                    style={{ width: `${percent}%` }}
                                  />
                                </div>
